...
# Helper methods
def format_df(df, feat, area=False):
    if not area:
        return df.loc[df.Year != 2020].groupby('PredPol Deployed')[feat].value_counts(normalize=True).unstack().T
    else:
        return df.loc[df.Year != 2020].groupby(['Area Name','PredPol Deployed'])[feat].value_counts(normalize=True).unstack()

def test(crime_tp, prop_pp, prop_nonpp, pct_pp=0.562854, n=100000):
    """
    Tests a single type of crime/arrest.
    """
    NUM_POP = n
    PCT_PREDPOL = pct_pp
